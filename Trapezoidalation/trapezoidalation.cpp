		...
		if(counter==nvertices)
			break;
				...
				if(flag2==0&&flag1==0)
				{
					...
					for(int i=0;i<edges.size();i++)
					{
						float xist= (float)((edges[i].p2.first-edges[i].p1.first)*Pointset[counter].y-((edges[i].p1.second*edges[i].p2.first)-(edges[i].p2.first*edges[i].p1.second)))/(edges[i].p2.first-edges[i].p1.first);
						if(xist>Pointset[counter].x){
							flag3=1;
							ptoinsert=i-1;
							if(i-1!=0)
							{
								float xist2= (float)((edges[i-1].p2.first-edges[i-1].p1.first)*Pointset[counter].y-((edges[i-1].p1.second*edges[i-1].p2.first)-(edges[i-1].p2.first*edges[i-1].p1.second)))/(edges[i-1].p2.first-edges[i-1].p1.first);
								line( img, Point((int)xist2,Pointset[counter].y), Point((int)xist,Pointset[counter].y ), Scalar( 110, 220, 0 ),  2, 8 );
